...
public class HeartDisplay extends GameActor
{
    ...
    private ArrayList<ForegroundPlayer> hearts = new ArrayList<ForegroundPlayer>();
    ...
    public HeartDisplay(DamageActor target, int maxHealth)
    {
        ...
        {
            ForegroundPlayer heart = new ForegroundPlayer(this);
    ...
    public void checkHearts()
    {
        if (target.getHealth() < prevHealth)
        {
            ...
            {
                for (int i = 0; i < 5; i++)
                    ((GameWorld) Global.world).freeAddObject(new HeartParticle(), getX() + (target.getHealth() + 1) * Options.HEART_SPACING, getY());
            }
            else
            {
                for (int i = 0; i < 5; i++)
                    ((GameWorld) Global.world).freeAddObject(new HeartParticle(), getX() + target.getHealth() * Options.HEART_SPACING, getY());
            }
        }
